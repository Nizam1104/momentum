...
const taskFormSchema = z.object({
    title: z.string().min(3, { message: 'Task title must be at least 3 characters.' }).max(150, { message: 'Task title cannot exceed 150 characters.' }),
    description: z.string().max(500, { message: 'Description cannot exceed 500 characters.' }).optional().nullable(),
    status: z.nativeEnum(TaskStatus, { message: 'Invalid task status.' }).default(TaskStatus.TODO),
    priority: z.nativeEnum(Priority, { message: 'Invalid priority.' }).default(Priority.MEDIUM),
    dueDate: z.date().optional().nullable(),
    estimatedMinutes: z.preprocess(
        (val) => (val === '' ? null : Number(val)),
        z.number().int().min(0, { message: 'Estimated minutes must be a positive number.' }).optional().nullable()
    ),
});
...
export const TaskForm: React.FC<TaskFormProps> = ({ initialData, projectId, onSubmit, onCancel }) => {
    ...
    const handleFormSubmit = (values: TaskFormValues) => {
        const taskData = {
            ...
            completedAt: values.status === TaskStatus.COMPLETED && !initialData?.completedAt ? new Date() : initialData?.completedAt || null,
        ...
        if (initialData) {
            ...
            onSubmit({ ...taskData, id: `temp_task_${Date.now()}` } as Task);
        }
    };
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <Input placeholder="e.g., Implement Project Card UI" {...field} />
                            </FormControl>
                            <FormMessage />
                        </FormItem>
                    )}
