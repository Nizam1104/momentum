...
// Category functions
export async function getAllCategories(userId: string): Promise<ActionResult<Category[]>> {
  try {
...
export async function getCategoryById(categoryId: string): Promise<ActionResult<Category>> {
  try {
    const supabase = await getSupabaseClient();
    const { data, error } = await supabase
      .from('Category')
...
export async function deleteCategory(categoryId: string): Promise<ActionResult<boolean>> {
  try {
    const supabase = await getSupabaseClient();
    
    // First, set categoryId to null for all related entities
    await Promise.all([
      supabase.from('Project').update({ categoryId: null }).eq('categoryId', categoryId),
      supabase.from('Task').update({ categoryId: null }).eq('categoryId', categoryId),
      supabase.from('Goal').update({ categoryId: null }).eq('categoryId', categoryId),
      supabase.from('Note').update({ categoryId: null }).eq('categoryId', categoryId),
    ]);
...
export async function deleteTag(tagId: string): Promise<ActionResult<boolean>> {
  try {
    ...
    await Promise.all([
      ...
      supabase.from('HabitTag').delete().eq('tagId', tagId),
    ]);
...
export async function addTagToNote(noteId: string, tagId: string): Promise<ActionResult<boolean>> {
  try {
    const supabase = await getSupabaseClient();
    const { error } = await supabase
      .from('NoteTag')
...
export async function removeTagFromNote(noteId: string, tagId: string): Promise<ActionResult<boolean>> {
  try {
    const supabase = await getSupabaseClient();
    const { error } = await supabase
      .from('NoteTag')
...
export async function addTagToProject(projectId: string, tagId: string): Promise<ActionResult<boolean>> {
  try {
    const supabase = await getSupabaseClient();
    const { error } = await supabase
      .from('ProjectTag')
...
export async function removeTagFromProject(projectId: string, tagId: string): Promise<ActionResult<boolean>> {
  try {
    const supabase = await getSupabaseClient();
    const { error } = await supabase
      .from('ProjectTag')
...
export async function addTagToTask(taskId: string, tagId: string): Promise<ActionResult<boolean>> {
  try {
    const supabase = await getSupabaseClient();
    const { error } = await supabase
      .from('TaskTag')
...
export async function removeTagFromTask(taskId: string, tagId: string): Promise<ActionResult<boolean>> {
  try {
    const supabase = await getSupabaseClient();
    const { error } = await supabase
      .from('TaskTag')
